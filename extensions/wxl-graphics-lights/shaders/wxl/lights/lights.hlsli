// wxl-graphics-lights: reading the light service's textures (GraphicsLightsApi.h LightTexture, ClusterTexture, OmniTexture) from a shader.
...
#ifndef WXL_LIGHTS_HLSLI
#define WXL_LIGHTS_HLSLI

// The caller binds both textures point-sampled and passes (WXL_GraphicsLightsApi::ClusterConstants):
...
// The cluster texture (A32B32G32R32F, every value exact) holds one texel per
...
// WXL_GFX_LIGHTS_MAX and the constant below, together.
static const float kLightCapacity = 128.0;
static const float kLightRows = 4.0;      // WXL_GFX_LIGHT_ROWS
static const float kRoomPad = 0.3;        // the room boxes' padding (Rooms)
...
// service's omni table (OmniTexture, row 0): the one place every pass reads it.
...
// 1 when the camera-relative point r lies in one room box (Rooms, three rows into the unit
...
// fourth row w: bit b of floor(flags / 4) is set for the rooms it may light (LightTexture row 3: its
