// wxl-graphics-lights: reading a light's baked cookie (GraphicsLightsApi.h CookieAtlas, the cookie rows of ClusterTexture) from a shader.
...
#ifndef WXL_LIGHTS_COOKIES_HLSLI
#define WXL_LIGHTS_COOKIES_HLSLI

#include "wxl/lights/lights.hlsli"

// Per light, two texels of the cluster texture past the per-cluster texels: row A the
...
// tint taken by cookieE.x (0 when the light has no cookie known). The atlas (CookieAtlas,
...
// atlas row (kCookiesAcross). Constants (WXL_GraphicsLightsApi::CookieConstants):
