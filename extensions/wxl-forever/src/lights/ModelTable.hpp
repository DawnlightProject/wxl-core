...
// data/model-lights.csv (tools/gen_model_lights.py), one row per light: model path stem (case and
// slash direction ignored), family, r, g, b, intensity, attenStart, attenEnd, then x, y, z in model
// space, and optionally three more: a tube light's half length in model space (windows, long
// fixtures), lit as an area by its closest point. Each row becomes a light placed by the instance's world matrix, for every unlit model
// instance of the world scene that the table names.
namespace wxl::forever::lights::table
{
    constexpr const char* kFile = "Extensions\\wxl-forever\\model-lights.csv";
