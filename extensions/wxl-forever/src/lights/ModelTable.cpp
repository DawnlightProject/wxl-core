// wxl-forever lights: the model table, light for models that carry none of their own.
...
namespace
{
    ...
    namespace tb = wxl::forever::lights::table;
    ...
    std::vector<wxl::forever::lights::Flicker> g_familyFlicker;
    std::vector<wxl::forever::lights::Profile> g_familyProfile;
    ...
    void ShapeOf(const std::string& family, wxl::forever::lights::Profile& profile, float& size)
    {
        using P = wxl::forever::lights::Profile;
    ...
    wxl::forever::lights::Flicker FlickerOf(const std::string& family)
    {
        using F = wxl::forever::lights::Flicker;
    ...
    uint64_t StemHash(const char* s, size_t maxLen) { return tb::StemHash(s, maxLen); }
    ...
    void Load()
    {
        ...
        {
            WLOG_WARN("lights: model table %s missing; models without lights stay dark", tb::kFile);
            return;
        }
        char line[512];
        ...
        while (std::fgets(line, sizeof line, f))
        {
            ...
            if (!comma) continue;
            const uint64_t key = StemHash(line, size_t(comma - line));
            char* p = std::strchr(comma + 1, ',');
            if (!p) continue;
            const std::string family(comma + 1, p);
            ...
            if (got != 9 && got != 12) continue;   // the header row, or a malformed one
            ...
            if (familyIndex == g_families.size())
            {
                ...
                wxl::forever::lights::Profile profile;
                float size;
                ShapeOf(family, profile, size);
                g_familyProfile.push_back(profile);
                g_familySize.push_back(size);
            }
            rows.push_back({ key, Row{ { v[0], v[1], v[2] }, v[3], v[4], v[5], { v[6], v[7], v[8] }, { v[9], v[10], v[11] },
                                       familyIndex } });
        }
        std::fclose(f);
        ...
        WLOG_INFO("lights: model table %s, %u rows for %u models, %u families", tb::kFile, unsigned(g_rows.size()),
...
namespace wxl::forever::lights::table
{
    size_t Collect(const float center[3], float radius, uint32_t maxStale, Light* out, size_t cap, Stats* stats)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            {
                const Row& row = g_rows[it->second.first + r];
