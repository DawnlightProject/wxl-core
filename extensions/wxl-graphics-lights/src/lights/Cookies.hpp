...
// wxl-forever's 5.tools/forever-bake renders, for each light of the model table, the transmittance of
// the model's own geometry seen from the light as a cube map in the model's local frame
...
namespace wxl::gfx::lights::cookies
{
    ...
    /// Reads WXL_GFX_LIGHTS_COOKIES_*; the manifest loads on first use. Called once at load.
    void Install();
    ...
    /// The atlas, or a 1 x 1 white texture while there is none (DEFAULT pool either way); null before
    /// the first Frame with a device.
    IDirect3DTexture9* Atlas();

    /// Shader constants (cookies.hlsli):
    ...
    /// Texels along one cube face of the atlas's cookies.
    unsigned FaceTexels();
    ...
    /// Debug view 1: draws the atlas in the top-left corner. From OnWorldRenderEnd.
    void DrawDebug(IDirect3DDevice9* dev);

    /// Frees the atlas and the white stand-in; both are rebuilt on the next Frame.
